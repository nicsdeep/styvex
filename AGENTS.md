...
### 3.3 Policy and Checkout Consistency (MANDATORY)
 - Read `docs/commerce-rules.md` before every commerce change. Supabase owns published retail prices: category markup once, with optional product price override. Shipping is separate: supplier freight plus 25% and $1.50 handling per order initially, editable by supplier in admin. Free standard U.S. shipping applies strictly above $600 after discounts excluding taxes/shipping; STYVEX pays supplier freight in full. Preserve these approved rules unless the owner changes them. Keep policies and payment enforcement synchronized; verify Edge Functions separately from Vercel. Never claim these repository instructions are global assistant memory.
