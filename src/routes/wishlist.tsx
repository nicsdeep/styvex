...
function WishlistPage() {
  ...
  const { data: wishlistedProducts, isLoading: dataLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((w) => w.products).filter((p): p is NonNullable<typeof p> => p !== null);
    },
    enabled: !!user,
  });
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 px-6 py-12 md:px-12 lg:px-24">
        <div className="mx-auto max-w-[1400px]">
          <h1 className="mb-12 text-3xl font-light uppercase tracking-widest text-foreground text-center">
            My Wishlist
          </h1>
          ...
          {!user ? (
            <div className="text-center py-12">
              ...
              <Link
                to="/account"
                className="inline-block bg-foreground px-8 py-3 text-xs font-semibold uppercase tracking-widest text-background transition-colors hover:bg-foreground/90"
              >
                Sign In
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {wishlistedProducts?.map((product: any) => {
                ...
                const primaryImage =
                  images[0]?.image_url ||
                  "https://placehold.co/400x500/eeeeee/999999?text=No+Image";
